...
func TestShouldCheckHeightBasedReboot(t *testing.T) {
	cases := []struct {
		...
		{
			name:          "peer responded and local node is genuinely behind",
			localHeight:   100,
			bestHeight:    200,
			hasResponded:  true,
			wantShouldRun: true,
		...
		{
			name:          "peer responded and local node is caught up",
			localHeight:   200,
			bestHeight:    200,
			hasResponded:  true,
			wantShouldRun: false,
		...
		{
			name:          "peer responded, local node is exactly at the lag threshold boundary",
			localHeight:   195,
			bestHeight:    200,
			hasResponded:  true,
			wantShouldRun: false,
		...
		{
			name:          "peer responded, local node is one block past the lag threshold",
			localHeight:   194,
			bestHeight:    200,
			hasResponded:  true,
			wantShouldRun: true,
		...
		{
			name:          "zero peers responded even when local height is huge",
			localHeight:   0,
			bestHeight:    0,
			hasResponded:  false,
			wantShouldRun: false,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := shouldCheckHeightBasedReboot(tc.localHeight, tc.bestHeight, tc.hasResponded)
			if got != tc.wantShouldRun {
				t.Errorf("shouldCheckHeightBasedReboot(%v, %v, %v) = %v, want %v",
					tc.localHeight, tc.bestHeight, tc.hasResponded, got, tc.wantShouldRun)
			}
		})
	}
}
